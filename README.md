...
|---|---|---|
| `talsecret.sops.yaml` | ✅ yes | Cluster CA and bootstrap tokens — SOPS-encrypted, generated by Terraform |
| `.sops.yaml` | ✅ yes | Age public key — written by Terraform on first apply |
| `age.key` | ❌ never | Age private key — generated by Terraform, stays local |
...
| Tool | Purpose |
|---|---|
| `age` + `sops` | Terraform uses these to generate and encrypt cluster secrets automatically |
...
1. Generate an age key pair → `age.key` (gitignored, stays local)
2. Write `.sops.yaml` with the generated public key
3. Generate and SOPS-encrypt cluster secrets → `talsecret.sops.yaml`
4. Render `talconfig.yaml` from your variables
5. Run `talhelper genconfig` → per-node machine configs in `clusterconfig/`
