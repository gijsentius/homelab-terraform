...
terraform apply
  ...
  ├── Generates cluster secrets (talos_machine_secrets)
  ├── Renders per-node machine configs from your variables
...
| `terraform.tfstate` | ❌ never | Terraform state — contains cluster CA/bootstrap secrets (`talos_machine_secrets`) and the ArgoCD SSH deploy key |
...
```bash
brew install terraform talosctl kubectl helm gh
```

| Tool | Purpose |
|---|---|
| `terraform` | Provision VMs and orchestrate the bootstrap (Terraform talks to the Talos API directly via the `siderolabs/talos` provider — no separate CLI needed for that) |
| `talosctl` | Optional — talk to Talos nodes directly for manual debugging (health checks, logs) |
...
1. Generate cluster secrets (`talos_machine_secrets`) — stored only in `terraform.tfstate`
2. Render per-node machine configs from your variables
...
5. Retrieve `kubeconfig` and `talosconfig`
